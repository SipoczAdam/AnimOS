...
void draw_desktop_icons(struct multiboot_tag_framebuffer* fb) {
    ...
    uint8_t* icon_datas[] = {file_explorer_icon_data, preferences_icon_data};

    for (int i = 0; i < 2; i++) {
        if (icon_datas[i]) {
            uint32_t icon_x = icon_xs[i], icon_y = icon_ys[i], icon_w = 48, icon_h = 48;
                ...
                if (dialog_state != 0) {
                    ...
                    if (mx >= (int32_t)start_btn_x && mx <= (int32_t)(start_btn_x + btn_w) && my >= (int32_t)btn_y && my <= (int32_t)(btn_y + btn_h)) {
                        ...
                        if (dialog_state == 1) {
                            ...
                            msleep(800);
                            for(uint32_t y = 0; y < fb->framebuffer_height; y++) for(uint32_t x = 0; x < fb->framebuffer_width; x++) draw_pixel(x, y, 0, fb);
                            const char* saving_msg = "Saving your settings...";
                            draw_string((screen_w - get_string_width(saving_msg)) / 2, screen_h / 2, saving_msg, 0xFFFFFF, fb);
                            msleep(800);
                            for(uint32_t y = 0; y < fb->framebuffer_height; y++) for(uint32_t x = 0; x < fb->framebuffer_width; x++) draw_pixel(x, y, 0, fb);
